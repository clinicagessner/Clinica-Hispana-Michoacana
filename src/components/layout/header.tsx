...
export function Header() {
  ...
  useEffect(() => {
    // Fuera de la home no hay scroll-spy. El hash anterior queda obsoleto pero
    // isActive() lo ignora (exige pathname === "/"), así que no hace falta
    // resetearlo aquí (evita setState síncrono dentro del efecto).
    if (pathname !== "/") return;
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-blue-deep/10 bg-white/90 backdrop-blur supports-backdrop-filter:bg-white/75">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 py-2.5 sm:px-6 lg:px-8">
        <Link
          href="/"
          aria-label="Clínica Hispana Nueva Salud Michoacana"
          className="shrink-0"
          onClick={(e) => {
