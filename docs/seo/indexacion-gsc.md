...
- 43 de 45 URLs del sitemap indexadas. Fuera: `/promociones` (descubierta, nunca rastreada)
  y el post de B12 (rastreado sin indexar).
- **El sitemap pasó de 45 a 90 URLs el 2026-09-22**: antes las páginas `/en` no se enviaban
  como URL propia, así que ninguna se ha pedido nunca.
- Todo el contenido se reescribió el 2026-09-22 y el `lastmod` es real.

## Cómo se pide

Search Console → Inspección de URLs → pegar la URL → **Solicitar indexación**.
Se marca la casilla al pedirla y se anota la fecha en el encabezado de la tanda.
Una tanda por día: Google admite 10 peticiones diarias por propiedad.

Orden: primero lo que Google no ha indexado, después el español por impresiones y al
